...
import { Clock, ArrowLeft, CheckCircle2, XCircle, AlertCircle, Trophy, RotateCcw, ShieldCheck, Star, HelpCircle, Check, X } from 'lucide-react';
...
export default function ExamMode({ quiz, testConfig, onBack }) {
  ...
  if (isSubmitted) {
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-8 text-warm-text">
        {/* Score Summary Box */}
        <motion.div
          initial={{ scale: 0.95, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          className="bg-white rounded-3xl p-8 border border-warm-border shadow-soft text-center space-y-6"
        >
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between border-b border-warm-border/60 pb-3">
            <h3 className="text-lg font-extrabold text-slate-900">Chi Tiết Xem Lại Bài Làm ({questions.length} câu)</h3>
            <span className="text-xs text-warm-muted">Bấm icon ⭐ để lưu các câu làm sai</span>
          </div>
          ...
          {questions.map((q, idx) => {
            ...
            let statusBadge = (
              <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-900 border border-emerald-300 px-3 py-1 rounded-full text-xs font-bold">
                <Check className="w-3.5 h-3.5 text-emerald-700" /> Chính xác
              </span>
            );
            ...
            if (isUnanswered) {
              statusBadge = (
                <span className="inline-flex items-center gap-1 bg-amber-100 text-amber-900 border border-amber-300 px-3 py-1 rounded-full text-xs font-bold">
                  <AlertCircle className="w-3.5 h-3.5 text-amber-700" /> Chưa trả lời
                </span>
              );
            } else if (!isCorrect) {
              statusBadge = (
                <span className="inline-flex items-center gap-1 bg-rose-100 text-rose-900 border border-rose-300 px-3 py-1 rounded-full text-xs font-bold">
                  <X className="w-3.5 h-3.5 text-rose-700" /> Chưa chính xác
                </span>
              );
            }
            ...
            return (
              <div
                key={q.id || idx}
                className="bg-white rounded-3xl p-6 sm:p-7 border border-warm-border shadow-xs space-y-5"
              >
                {/* Header Row */}
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-warm-muted">Câu hỏi {idx + 1}</span>
                    <h4 className="text-base sm:text-lg font-bold text-slate-900 leading-relaxed break-words">
                      {q.content}
                    </h4>
                  </div>
                ...
                <div className="space-y-2.5">
                  {(q.answers || []).map((answer, aIdx) => {
                    ...
                    let optionStyle = "bg-gray-50/50 border-gray-200 text-gray-400 opacity-60";
                    ...
                    if (isUserChoice && isRightAnswer) {
                      ...
                      optionBadge = (
                        <span className="flex items-center gap-1 text-xs font-bold text-emerald-700">
                          <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Bạn chọn đúng
                        </span>
                      );
                    } else if (isUserChoice && !isRightAnswer) {
                      // Wrong answer chosen by user
                      optionStyle = "bg-rose-50 border-rose-500 text-rose-950 font-semibold shadow-xs";
                      optionBadge = (
                        <span className="flex items-center gap-1 text-xs font-bold text-rose-700">
                          <XCircle className="w-4 h-4 text-rose-600" /> Bạn chọn sai
                        </span>
                      );
                    } else if (isRightAnswer && !isCorrect) {
                      // Standard correct answer when user chose wrong or skipped
                      optionStyle = "bg-emerald-50/60 border-2 border-dashed border-emerald-400 text-emerald-900 font-semibold";
                      optionBadge = (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <span className="w-5 h-5 rounded-full bg-black/5 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                ...
                {q.explanation && (
                  <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200 text-xs text-amber-950 leading-relaxed">
                    <span className="font-bold text-amber-900 block mb-1">💡 Giải thích chi tiết:</span>
                    {q.explanation}
                  </div>
                )}
              </div>
